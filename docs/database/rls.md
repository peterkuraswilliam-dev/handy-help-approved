...
The implemented invitation migration grants authenticated admins read access to invitation rows and exposes admin-checked create/revoke functions. Signed-out account creation uses service-role-only reservation/completion functions through a validated server function; existing authenticated users use a narrowly scoped atomic acceptance function. Direct public table access is not granted. Contractor application insert/update policies additionally require the server-controlled contractor role.
